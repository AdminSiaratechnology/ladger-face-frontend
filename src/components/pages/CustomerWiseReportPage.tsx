...
const CustomerWiseReportPage = () => { 
  ...
  console.log("Default Selected Company:", defaultSelected);
...
useEffect(() => { 
  ...
  const statCards = useMemo( 
    () => [ 
      ...
      { 
        label: "Total Sales", 
        value: `${defaultCurrency} ${stats?.totalSales?.toLocaleString() || 0}`, 
        icon: <TrendingUp className="w-8 h-8" />, 
        colorClass: "from-teal-500 to-teal-600", 
      }, 
      { 
        label: "Total Received", 
        value: `${defaultCurrency} ${stats?.totalReceived?.toLocaleString() || 0}`, 
        icon: <TrendingUp className="w-8 h-8" />, 
        colorClass: "from-green-500 to-green-600", 
      }, 
      { 
        label: "Outstanding", 
        value: `${defaultCurrency} ${stats?.outstanding?.toLocaleString() || 0}`, 
              ...
              {transactions.map((t: any) => ( 
                <tr key={t._id} className="hover:bg-gray-50"> 
                  ...
                  <td className="px-6 py-4 text-right text-red-600"> 
                    {t.orderAmount 
                      ? `${defaultCurrency} ${t.orderAmount.toLocaleString()}` 
                      : "-"} 
                  </td> 
                  <td className="px-6 py-4 text-right text-green-600"> 
                    {t.paymentAmount 
                      ? `${defaultCurrency} ${t.paymentAmount.toLocaleString()}` 
                      : "-"} 
                  </td> 
                  <td className="px-6 py-4"> 
                    <Badge 
